...
export class ProyectosUsuarioCardComponent {
  ...
  toggleFavorita() {
    ...
    this.proyectoService.cambiarEstadoFavorito(this.favorita)
      .subscribe(() => {
  ...
  onDeleteClicked(id: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.proyectoService.cancelarProyecto(id)
          .subscribe({
            next: () => {
  ...
  verProyecto() {
    if (!this.esEditable) {
      this.router.navigate([`/usuario/proyectos/${this.proyectoUnico.idProyecto}`]);
    }
  }
}